...
class GetkAdapterImpl;

/**
* @class GetkAdapter
* The GETK adapter realizes the interface between the AOS MTA Gateway and the GETK.
*/
class GetkAdapter : public api::IGetkAdapter
{
private:
    std::unique_ptr<GetkAdapterImpl> m_impl;

    GetkAdapter(const GetkAdapter&) = delete;
    GetkAdapter& operator=(const GetkAdapter&) = delete;
